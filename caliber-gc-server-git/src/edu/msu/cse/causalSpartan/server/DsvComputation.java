...
import edu.msu.cse.dkvf.metadata.Metadata.GVTMessage;
...
public class DsvComputation implements Runnable {
	...
	private void garbageCollect() {
		if(server.garbageCollect == 1) {
			...
			ServerMessage sm = ServerMessage.newBuilder().setGvtMessage(GVTMessage.newBuilder().setDcId(server.dcId).setGvtItem(server.gvt.get(server.dcId))).build();
