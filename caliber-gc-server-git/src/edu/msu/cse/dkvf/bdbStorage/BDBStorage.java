...
public class BDBStorage extends Storage {
	...
	/**
	 * Delete the  version of the data item with the given key that satisfies the given predicate.
	 * @param key The key of the data item to read. 
	 * @param p The predicate that need to be satisfied by the version.   
	 * @return The result of the operation
	 */
	public StorageStatus delete(String key, Predicate<Record> p) {
		...
		try {
			...
			if (db != null)
				...
				logger.severe("Problem in delete. DB is not running properly.");
				return StorageStatus.FAILURE;
			}
			// Position the cursor
			OperationStatus retVal = cursor.getSearchKey(myKey, myData, LockMode.DEFAULT);
			OperationStatus retValue = OperationStatus.SUCCESS;
			while (retVal == OperationStatus.SUCCESS && retValue == OperationStatus.SUCCESS) {
				Record rec = Record.parseFrom(myData.getData());
				if (p.test(rec)) {
					retValue = cursor.delete();
				}
				retVal = cursor.getNextDup(myKey, myData, LockMode.DEFAULT);
			}
			cursor.close();
			if(retValue == OperationStatus.SUCCESS)
				return StorageStatus.SUCCESS;
			else
				return StorageStatus.FAILURE;
			
		} catch (Exception e) {
			logger.severe("Problem in deleting versions in key= " + key + " : " + e.toString());
			return StorageStatus.FAILURE;
		} finally {
			if (cursor != null)
				cursor.close();
	...
	public String getDbAsString() {
		...
		try {
			...
			while (cursor.getNext(foundKey, foundData, LockMode.DEFAULT) ==
				...
				Record record =Record.parseFrom(foundData.getData());
				result = result +  keyString + " | " + record.getUt() + " | " + record.getSr() + " | " + (record.getDsItemList()).toString()+ "\n";
			}
		} catch (Exception de) {
			logger.severe("Error accessing database." + de);
		} finally {
	    	cursor.close();
		}
		return result;
	...
	public StorageStatus getKeys(List<String> result) {
		...
		try {
			...
			while (cursor.getNextNoDup(foundKey, foundData, LockMode.DEFAULT) ==
				OperationStatus.SUCCESS) {
				String keyString = new String(foundKey.getData());
